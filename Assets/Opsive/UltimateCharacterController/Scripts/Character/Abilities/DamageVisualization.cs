...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public class DamageVisualization : Ability
    {
        ...
        public enum TakeDamageIndex {
            ...
            Swept,   // Play a sweep animation.
            FallBack,   // Play a fall animation.
            FallForward   // Play a fall animation.
        }
        ...
        {
            Debug.Log("damage collider: "+ hitCollider.gameObject.transform.parent.name + " damage: " + amount);
            ...
            if (hitCollider.ToString() == "CC_Base_Pelvis")
            {
                Debug.Log("pelvis!");
            }
            ...
            m_TakeDamageIndex = GetDamageTypeIndex2(amount, position, force, attacker);
        ...
        protected virtual int GetDamageTypeIndex2(float amount, Vector3 position, Vector3 force, GameObject attacker)
        {
            var direction = m_Transform.InverseTransformPoint(position);
            ...
            if (direction.z > 0)
            {
                if (amount > 20)
                {
                    //Debug.Log("fall back: " + (int)TakeDamageIndex.FallBack);
                    return (int)TakeDamageIndex.FallBack;
                }
                else
                {
                    if (direction.x > 0)
            ...
            else if (direction.z < 0)
            {
                if (amount > 20)
                {
                    ...
                    return (int)TakeDamageIndex.FallForward;
                }
                else
                {
                    if (direction.x > 0)
